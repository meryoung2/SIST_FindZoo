...
<script type="text/javascript">
	function btn_start(search_option, keyword){
		location.href = "searchFindReview.do?pageNum=1"+"&search_option="+search_option+"&keyword="+keyword;
	}
	
	function btn_end(searchPage, search_option, keyword){
		location.href = "searchFindReview.do?pageNum="+searchPage+"&search_option="+search_option+"&keyword="+keyword;
	}
	...
	function btn_prev(s_listStart, s_listEnd, search_option, keyword){
		...
		location.href = "searchFindReview.do?pageNum="+s_listEnd+"&search_option="+search_option+"&keyword="+keyword;
	}
	...
	function btn_next(s_listStart, s_listEnd, searchPage, search_option, keyword){
		if(s_listEnd != searchPage){
			...
			location.href = "searchFindReview.do?pageNum="+s_listStart+"&search_option="+search_option+"&keyword="+keyword;
		}else{
			location.reload();
		}
	}
...
<jsp:include page="findZoo_Header.jsp"/>
	<div id="free-container">
		<div id="content">
			<h2><a href="findReview.do">찾은 후기 게시판</a></h2>
			...
			<div id="board">
				<table border="1" class="table table-hover">
					...
					<tbody>
						<c:forEach var="s_f" items="${ list }">
							<tr>
								<td width="60%"><a
									href="detailFindReview.do?board_num=${ s_f.board_num }">${ s_f.title }</a>
								</td>
								<td width="30%">
				...
				<button class="btn btn-primary" onclick="location.href='insertFindReview.do'" style="float: right;">글쓰기</button>
			</div>
				...
				<c:forEach var="i" begin="${ s_listStart }" end="${ s_listEnd }">
					<c:choose>
						<c:when test="${ pageNum eq i }">
							<a href="searchFindReview.do?pageNum=${ i }&search_option=${ search_option }&keyword=${ keyword }" style="font-weight: bold; color: #325d88; text-decoration: underline;">[${ i }]</a>&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
							<a href="searchFindReview.do?pageNum=${ i }&search_option=${ search_option }&keyword=${ keyword }">[${ i }]</a>&nbsp;&nbsp;
						</c:otherwise>
					</c:choose>
				</c:forEach>
